// 文件功能: 提供通用指标度量与上报框架，支持周期性聚合请求任务、耗时统计、分位数(50/90/99/99.9)计算与自定义上报。
// 关键技术点:
// - 周期执行器: 通过 `executors.PeriodicalExecutor` 定期聚合并输出统计信息。
// - 锁与原子: 使用互斥锁/原子布尔保证并发安全（写入器、日志开关）。
// - 分位数估计: 通过 topK 的简化策略计算中位数与高分位值。
// - 可插拔上报: 允许设置 `Writer` 将统计上报到外部系统。
// 适用场景: RPC/HTTP 调用耗时与 QPS 统计、降级与熔断策略的观测基线。
package stat
