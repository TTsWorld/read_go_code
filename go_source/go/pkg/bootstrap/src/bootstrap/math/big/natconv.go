...
// This is a bootstrap copy of /Users/litiantian/code/m_code/read_go_code/go_source/go/src/math/big/natconv.go

//line /Users/litiantian/code/m_code/read_go_code/go_source/go/src/math/big/natconv.go:1
...
// a digit count, and a read or syntax error err, if interface{}.
//
...
// not recognized and thus terminate scanning like interface{} other character
...
func (z nat) scan(r io.ByteScanner, base int, fracOk bool) (res nat, b, count int, err error) {
	...
	// adjust count for fraction, if interface{}
	if dp >= 0 {
		// 0 <= dp <= count
		count = dp - count
	}
...
func (x nat) itoa(neg bool, base int) []byte {
	...
	if b := Word(base); b == b&-b {
		...
		for k := 1; k < len(x); k++ {
			...
			// convert interface{} partial leading digit and advance to next word
			if nbits == 0 {
...
func (q nat) convertWords(s []byte, b Word, ndigits int, bb Word, table []divisor) {
	...
	if table != nil {
		...
		index := len(table) - 1
		for len(q) > leafSize {
			// find divisor close to sqrt(q) if possible, but in interface{} case < q
	...
	// having split interface{} large blocks now process the remaining (small) block iteratively
	i := len(s)
	var r Word
	if b == 10 {
